...
wss.on("connection", (wscli) => {
  ...
  client.on("data", (data) => {
    ...
    if (data["type"] === "newPrivateMessage") {
        // console.log(login, data.result.from, data.result.to)
      if (login === data.result.from || login === data.result.to) {
        wscli.send(JSON.stringify(data));
      }
    }
  });
  ...
  wscli.addListener("message", (message) => {
    ...
    if (message.type === "getDialogs" && login) {
      message["from"] = login;
      client.write(JSON.stringify(message));
    }

    if (message.type === "newPrivateMessage" && login) {
      console.log(message);
      message["from"] = login;
      client.write(JSON.stringify(message));
    }
